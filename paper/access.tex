...
\begin{itemize}
  \item{Anonymous payments} — we use the Monero blockchain \cite{monero-blockchain}.
  \item{Storage netowork} — we use the Powergate, which is a wrapper around Filecoin and IPFS \cite{ipfs}.
  \item{Message board} — we use the Ethereum blockchain \cite{ethereum-blockchain}; concretely, Truffle Ganache and Solidity language.
...
Moreover, Powergate (storage network) is configured to use local Filecoin and IPFS networks.
...
1. The protocol starts with the Customer opening the webapp and creating new provision. 
...
2. The Customer prints the QR code and put it on a package.

3. The Customer delivers his package to the SP either: in person, via trusted party, or delivery agency.

4. The SP opens the app, scans the QR code decoding the provisionID and the client's public key.

5. Since (in this experiment) the provision has not been paid in cash, the SP submits a Proof of Delivery transaction to the Ethereum blockchain, indicating that fact setting the `paid` flag to `false`, and `paymentAddress` field with dedicated Monero address using `monero-wallet-cli integrated-address`.
...
6. The Customer (using the webapp) check the transaction status on the Ethereum blockchain invoking `getProvision(clientPubKey, provisionID)`.

7. The Customer (using the `monero-wallet-cli transfer`) sends the payment to the designated in the smart contract `paymentAddress` and stores the .

8. The SP starts providing the service and outputs the file `result.pdf`.

9. The SP uploads the file `result.pdf` on the Filecoin and IPFS networks using Powergate. In result, SP gets the content identifier (`cid`), and `dealID`.

10. The SP submits a Proof of Provision transaction to the Ethereum blockchain by invoking `proofOfProvision(clientPubKey, provisionID, cid, dealID)`.

11. Meantime, the Customer subscribes to the Ethereum and waits until the SP publishes the Proof Of Provision.
...
The possible directions for mitigation of such issues are to either (i) replace justice with decentralized autonomous organizations (DAO) like proposed in Themis~\cite{meng2019themis} or Kleros~\cite{lesaege2018kleros}; or (ii) make it infeasible to provide incorrect results.
...
We assumed the customer to deliver the package to the SP personally or via a trusted person. However, the system could be extended to support an anonymous delivery system as proposed in Lelantos~\cite{altawy2017lelantos}. One modification to the protocol would be to publish $\mathrm{PoD}$ on the message board, similarly to $\mathrm{PoP}$.
...
\item
  ...
  \item
    \(\operatorname{\sigma_{3, c, \n} = -}\), the customer has not received the result, so he remains in the disadvantaged position. 
  \item
